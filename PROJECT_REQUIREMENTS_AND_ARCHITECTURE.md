...
本节用于覆盖前文中已经过时的状态描述。若与前文冲突，以本节为准。

### 15.1 大窗口模式与对话链路

- 主聊天链路已经接入主进程到 renderer 的真实流式输出，不再是前端整段返回后的伪打字机效果。
- assistant 侧已经支持取消、超时、重试。
- Agent 过程展示已经从纯 trace 文本升级为折叠式执行摘要面板。
- 当前仍未完成的部分：
  - 还不是完整任务树面板。
  - 还没有步骤耗时、参数级展示和工具级独立超时。
  - 还缺少系统化回归测试和完整 Electron 联调验证。

### 15.2 Persona 与普通聊天 / Agent 链路

- 普通 `aiChat` 与 `agentChat` 已统一使用同一份 persona 来源。
- Agent 最终回答会继承人设风格，但工具调用阶段仍保持结构化约束，不混入 persona 文本。

### 15.3 设置页与 Agent 自检

- Agent 自检不再只依赖聊天关键词触发。
- 设置页已经提供显式 `Run self-test` 入口，并展示：
  - 总工具数
  - 成功 / 失败统计
  - 单工具输入与输出摘要
- 聊天主链路中的关键词触发入口已删除，避免普通对话误触发自测。

### 15.4 资料库与 PDF 边界

- PDF 已进入正文抽取、摘要、chunk 构建和索引链路。
- `search_library` 已可命中 PDF 正文片段，不再只依赖文件名和元数据。
- `read_library_file` 对 PDF 已返回：
  - 抽取文本
  - 索引命中片段
  - 原文件路径
  - 文本抽取失败时的 warning
- organizer 预览页对 PDF 已支持三段式展示：
  - Extracted text
  - Indexed matches
  - Original file
- 当前仍未完成的部分：
  - 没有 OCR，扫描件或纯图片 PDF 仍无法进入正文检索。
  - 检索仍是关键词 / 规则召回，不是 embedding 语义检索。
  - 还没有结果重排和更稳定的相关性排序。
...
1. 本地语义 RAG（embedding + rerank）
2. Agent 多步执行状态机与失败恢复
3. 资料检索排序与摘要质量提升
4. 最小回归测试补齐

### 15.6 Agent Prompt 与数据流边界

- Agent 链路已不再与助手最终回答共用同一段 system prompt。
- 当前实现已拆分为两层：
  - `agent planning prompt`：只负责内部思考、工具选择、工具调用与内部草稿生成
  - `assistant final answer prompt`：只负责基于已有结果生成人设化最终回复
- 当前 `agentChat` / `agentChatStream` 的数据流为：
  1. 用户消息进入 planning conversation
  2. Agent 在思考模式中决定是否调用工具
  3. 每一步 planning 产出摘要化 workflow 节点
  4. 工具调用结果写回 planning conversation 与 workflow
  5. Agent 产出内部草稿或结束信号
  6. 主链路再构造 final-answer conversation
  7. 助手 persona 仅在 final-answer conversation 中参与最终输出
- 这意味着：
  - 用户可见聊天气泡不再直接暴露 tool JSON、内部编排旁白或测试式提示词输出
  - 上方 workflow 面板仍保留思考过程，但展示的是摘要化节点，不是原始隐藏推理文本
- 当前 workflow 已支持两类节点：
  - `thinking` 摘要节点
  - `tool` 执行节点
- 当前仍未完成的部分：
  - workflow 节点仍偏工程化，尚未完全中文化
  - thinking 摘要模板还可以进一步自然化
  - 还没有针对多工具长链路做系统化端到端验证
